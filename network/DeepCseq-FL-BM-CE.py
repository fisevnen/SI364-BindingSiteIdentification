...
class NetWork(nn.Module): 
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 512, (5,45), padding=(2,0))
        self.block1 = nn.ModuleList([
            nn.LayerNorm((600,1)), #self.block[0]
            nn.GLU(dim=1),#self.block[1]
            nn.Conv2d(256, 512, (5,1), padding=(2,0)) #self.block[2]
        ])
        for i in range(9):
            self.block1.append(nn.LayerNorm((600,1)))
            self.block1.append(nn.GLU(dim=1))
            self.block1.append(nn.Conv2d(256, 512, (5,1), padding=(2,0)))
        ...
        self.lastconv1 = nn.Conv2d(256, 256, 1)
        self.lastconv2 = nn.Conv2d(256, 2, 1)
...
def train_loop(dataloader, model):
    loss_fn = focal_loss(alpha=[0.5, 0.5], gamma=0, num_classes=2)
